 ...
 * @since 1.0.0
 */
function csco_mega_menu_child_init() {
	new CSCO_Mega_Menu_Child();
}
add_action( 'init', 'csco_mega_menu_child_init' );

/* Remove parent theme mega menu class init */
function remove_mega_menu_actions() {
    global $wp_filter;
    if( array_key_exists( 'csco_mega_menu_init', $wp_filter[ 'init' ][ 10 ] ) ) {
        remove_action( 'init', $wp_filter[ 'init' ][ 10 ][ 'csco_mega_menu_init' ][ 'function' ] );
    }
}
add_action( 'after_setup_theme', 'remove_mega_menu_actions' );